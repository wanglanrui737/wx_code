...
from data_load import get_batch_data, load_dev_data
from modules import *
...
class Graph():
    def __init__(self, hp, is_training=True):
        ...
        with self.graph.as_default():
            if is_training:
                self.x, self.x_image, self.y_image, self.x_length, self.y, self.num_batch, self.source, self.target, \
                self.x_turn_number, self.x_emotion, self.y_emotion, self.speaker, self.A = get_batch_data(
                    hp)  # (N, T)
            else:  # inference
            ...
            with tf.variable_scope("encoder"):
                ...
                # elf.rnn_cell = tf.nn.rnn_cell.MultiRNNCell([single_cell] * hp.num_layers)
                ...
                self.uttn_outputs, self.uttn_states = tf.nn.dynamic_rnn(cell=self.rnn_cell, inputs=self.enc_embed,
                                                                        sequence_length=self.sequence_length,
                                                                        dtype=tf.float32, swap_memory=True)

                print(hp.batch_size, hp.max_turn, hp.hidden_units)

                self.enc = tf.reshape(self.uttn_states,
                                      [hp.batch_size, hp.max_turn, hp.hidden_units])  # shape=(16, 15, 512)
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        self.enc = feedforward(self.enc,
                                               num_units=[4 * hp.hidden_units, hp.hidden_units])  # shape=(32, 15, 512),
                        # code.interact(local=locals())
            matrix = tf.get_variable("transform", [self.x_image.shape.as_list()[-1], self.enc.shape.as_list()[-1]],
                                     dtype=tf.float32)
            self.x_ima = tf.map_fn(lambda x: tf.matmul(x, matrix), self.x_image, dtype=tf.float32)
            # code.interact(local=locals())
            self.enc = tf.concat((self.enc, self.x_ima), -2)  # utterance与图像向量
            ...
            self.enc = tf.concat((self.enc, e_m), -2)  # 与情绪向量
            self.enc = tf.concat((self.enc, s_m), -2)  # 与spe向量
            ...
            with tf.variable_scope("emotion"):
                text_image = tf.map_fn(lambda x: x[:-1], self.enc, dtype=tf.float32)  # 扔掉了speaker
            ...
            self.preds = tf.to_int32(tf.arg_max(self.logits, dimension=-1))  # shape=(128, 50) # 这里最长的回复是128
            ...
            self.loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits,
                                                                labels=self.y_smoothed)  # shape=(256, 50)
            ...
            if is_training:
                ...
                self.train_op = self.optimizer.minimize((1 - hp.alpha) * self.mean_loss + hp.alpha * emotion_loss,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_train', type=str, default=base_dir + 'corpora/train_answer.txt',
                        help='src train file')
    ...
    SRC_emotion, TGT_emotion, Speakers, A = load_dev_data(hp)
    # Start session
            ...
            for step in range(g.num_batch):
                ...
                if step % 500 == 0:
                    ...
                    for i in range(len(X) // hp.batch_size):
                        ...
                        eval_bath = sess.run(g.mean_loss, {g.x: x, g.x_image: x_image, g.x_length: x_length, g.y: y,
                                                           g.x_emotion: x_emotion, g.speaker: speaker, g.A: a,
                                                           g.x_turn_number: x_turn_number})
                        eval_loss.append(eval_bath)

                        viterbi_sequence = sess.run(g.emotion_logits,
                                                    {g.x: x, g.x_image: x_image, g.x_length: x_length, g.y: y,
                                                     g.x_emotion: x_emotion, g.speaker: speaker, g.A: a,
                                                     g.x_turn_number: x_turn_number})
                        pred_emotion.extend(
                            viterbi_sequence.tolist())  # = tf.map_fn(lambda x: x[0][x[1]], (viterbi_sequence, X_turn_number), dtype = tf.int32)

                        preds = np.zeros((hp.batch_size, hp.maxlen), np.int32)
                        for j in range(hp.maxlen):
                            _preds = sess.run(g.preds, {g.x: x, g.x_image: x_image, g.x_length: x_length, g.y: preds,
                                                        g.x_emotion: x_emotion, g.speaker: speaker, g.A: a,
                                                        g.x_turn_number: x_turn_number})
                            preds[:, j] = _preds[:, j]
                        ...
                        for source, target, pred in zip(sources, targets, preds):  # sentence-wise
                            ...
                            score = nltk.translate.bleu_score.sentence_bleu([hypothesis], ref, (0.25, 0.25, 0.25, 0.25),

...
def load_de_vocab(hp):
    vocab = [line.split()[0] for line in
             codecs.open(base_dir + 'preprocessed/de.vocab.tsv', 'r', 'utf-8').read().splitlines() if
...
def load_en_vocab(hp):
    vocab = [line.split()[0] for line in
             codecs.open(base_dir + 'preprocessed/en.vocab.tsv', 'r', 'utf-8').read().splitlines() if
             int(line.split()[1]) >= hp.min_cnt]
    word2idx = {word: idx for idx, word in enumerate(vocab)}
    idx2word = {idx: word for idx, word in enumerate(vocab)}
    return word2idx, idx2word
...
def load_speaker_vocab(hp):
    vocab = [line.split('\n')[0] for line in
             codecs.open(base_dir + 'preprocessed/speakers.txt', 'r', 'utf-8').read().splitlines()]
    word2idx = {word: idx for idx, word in enumerate(vocab)}
    idx2word = {idx: word for idx, word in enumerate(vocab)}
    return word2idx, idx2word
...
def load_image_data(hp, data_type):
    image_features = [line for line in
                      codecs.open(hp.corpora_path + data_type + '_image.txt', 'r', 'utf-8').read().split("\n") if line]
...
def load_train_data(hp):
    ...
    X, X_image, Y_image, X_length, Y, Sources, Targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A = create_data(
        hp, de_sents, en_sents, image_fea, A)
...
# g = Graph(hp, "train");
# print("Graph loaded")
X, X_image, Y_image, X_length, Y, Sources, Targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A = load_train_data(
    hp)
data_list = [X, X_image, Y_image, X_length, Y, Sources, Targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A]

# print(Sources)


str_list = ["X", "X_image", "Y_image", "X_length", "Y", "Sources", "Targets", "X_turn_number", "SRC_emotion",
            "TGT_emotion", "Speakers", "A"]
for x in range(len(data_list)):
    try:
        print(str_list[x], "： ", data_list[x].shape)
    except:
        print(str_list[x], "： ", len(data_list[x]))
...
#         [X, X_image, Y_image, X_length, Y, sources, targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A])
#
#     # create batch queues
#     x, x_image, y_image, x_length, y, sources, targets, X_turn_number, SRC_emotion, TGT_emotion, Speakers, A = tf.train.shuffle_batch(
#         input_queues,
#         num_threads=8,
